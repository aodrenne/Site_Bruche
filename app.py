...
def show_point_popup(obj):
    ...
    if photo and str(photo).lower() != "nan":
        ...
        # S'assure que l'extension a bien son point (ex: .jpg)
        if photo_name.lower().endswith("jpg") and not photo_name.lower().endswith(".jpg"):
            photo_name = photo_name[:-3] + ".jpg"
        elif photo_name.lower().endswith("png") and not photo_name.lower().endswith(".png"):
            photo_name = photo_name[:-3] + ".png"
            
        # URL directe Hugging Face
        photo_url = f"https://huggingface.co/datasets/AodRic/Data_SIP/resolve/main/static/{photo_name}"
        
        # TEST DE DÉBOGAGE : Affiche l'URL en texte pour voir si elle est correcte
        st.text(f"URL testée : {photo_url}")
        
        # Affichage direct de l'image par son URL
        st.image(photo_url)
    else:
        st.warning("Aucune photo renseignée pour ce point.")
...
def show_commune_popup(props):
    ...
    if blason and blason.lower() != "nan":
        blason_name = blason
        if blason_name.endswith("png") and not blason_name.endswith(".png"):
            blason_name = blason_name[:-3] + ".png"
        elif blason_name.endswith("jpg") and not blason_name.endswith(".jpg"):
            blason_name = blason_name[:-3] + ".jpg"
        ...
        with col2:
            try:
                st.image(blason_url, width=100)
            except Exception:
                st.warning(f"Blason introuvable : {blason_name}")
    else:
        st.warning(f"Blason non renseigné")
