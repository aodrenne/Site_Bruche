...
def show_commune_popup(props):
    ...
    if blason and blason.lower() != "nan":
        # On récupère le nom et l'éventuelle extension existante
        name, ext = os.path.splitext(blason)
        
        # Si aucune extension n'est trouvée dans le CSV, on force '.png' par défaut (change en '.jpg' si besoin)
        if not ext:
