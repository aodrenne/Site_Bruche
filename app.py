...
def show_point_popup(obj):
    photo = obj.get("Photo", "")
    if photo and photo != "nan":
        photo_stem = Path(photo).stem.strip()
        for ext in [".jpg", ".jpeg", ".png"]:
            photo_path = Path(__file__).parent / "static" / (photo_stem + ext)
...
def show_commune_popup(props):
    ...
    if blason and blason != "nan":
        blason_stem = Path(blason).stem.strip()
        for ext in [".jpg", ".jpeg", ".png"]:
            photo_path = Path(__file__).parent / "static" / "blason" / (blason_stem + ext)
            if photo_path.exists():
